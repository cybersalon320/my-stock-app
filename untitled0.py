...
"""
for i, item in enumerate(schedule):
    style = "background:#A3B18A; color:white; border-radius:10px; padding:10px;" if i == hi_idx else "padding:10px; border-bottom:1px solid #eee;"
    html += f'<div style="{style} display:flex; justify-content:space-between; margin-top:5px;"><span>{item["name"]}</span><span>{item["start"]}-{item["end"]}</span></div>'

html += f"""
        </div>
        <div style="background: white; padding: 30px; border-radius: 20px; flex: 1.5; text-align: center;">
            <div style="color: #BC8F8F; font-size: 24px; font-weight: bold; letter-spacing: 10px;">考 場 規 範</div>
            <h1 style="font-size: 48px; margin: 30px 0;">🚫 考完請在位靜候<br><span style="font-size: 30px; color: #666;">等監考老師收完卷</span></h1>
            <div style="display: flex; justify-content: space-around; background: #FDF5E6; padding: 20px; border-radius: 15px;">
                <div><small>應到</small><br><b style="font-size: 40px;">{st.session_state.total}</b></div>
                <div><small>實到</small><br><b style="font-size: 40px;">{st.session_state.present}</b></div>
                <div><small>缺席</small><br><b style="font-size: 40px; color: {'#BC8F8F' if absent > 0 else '#5D5D5D'};">{absent}</b></div>
            </div>
        </div>
    </div>
</div>
